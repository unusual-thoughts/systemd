...
static void test_translate_name(void) {
        ...
        assert_se(streq(t, "apps-a\\x2db.blub-autostart.service"));
}
...
static void test_xdg_format_exec_start(void) {
        test_xdg_format_exec_start_one("/usr/bin/sleep 100", "/usr/bin/sleep \"100\"");

        /* All standardised % identifiers are stripped. */
        test_xdg_format_exec_start_one("/usr/bin/sleep %f \"%F\" %u %U %d %D\t%n %N %i %c %k %v %m", "/usr/bin/sleep");

        /* Unknown % identifier currently remain, but are escaped. */
        test_xdg_format_exec_start_one("/usr/bin/sleep %X \"%Y\"", "/usr/bin/sleep \"%%X\" \"%%Y\"");

        test_xdg_format_exec_start_one("/usr/bin/sleep \";\\\"\"", "/usr/bin/sleep \";\\\"\"");
}
...
static void test_xdg_desktop_parse(unsigned i, const char *s) {
        ...
        switch (i) {
        case 0:
                assert_se(streq(service->exec_string, "/usr/bin/sleep 100"));
                ...
                break;
        case 1:
                /* The second entry is not permissable and will be ignored (and error logged). */
